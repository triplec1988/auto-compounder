...
/* 
    Source: https://github.com/Badger-Finance/badger-system/blob/master/contracts/badger-sett/Sett.sol
    
    Inspired by the BadgerDAO Sett
*/

contract Cellar is ERC20Upgradeable, CellarAccessControlAuthorizer, PausableUpgradeable {
    ...
    /// @notice Sums the balance in the Cellar, the Controller, and the Strategy
    function balance() public view returns (uint256) {
        return token.balanceOf(address(this)).add(IController(controller).balanceOf(address(token)));
    }

    /// @notice Defines how much of the Cellars' underlying can be borrowed by the Strategy for use
    ...
    /// @notice Deposit assets into the Cellar, and return corresponding shares to the user
    /// @notice Only callable by EOA accounts that pass the _authorized() check
    ...
    /// @notice Convenience function: Deposit entire balance of asset into the Cellar, and return corresponding shares to the user
    /// @notice Only callable by EOA accounts that pass the _authorized() check
    ...
    /// @dev This is based on the realized value of underlying assets between Cellar & associated Strategy
