...
import "../../../interfaces/cellar/IStrategy.sol";
...
*/
abstract contract BaseStrategy is PausableUpgradeable, CellarAccessControl {
    ...
    function _swap(
        ...
        IUniswapRouterV2(uniswap).swapExactTokensForTokens(balance, 0, path, address(this), block.timestamp);
    }

    function _swapEthIn(uint256 balance, address[] memory path) internal {
        IUniswapRouterV2(uniswap).swapExactETHForTokens{value: balance}(0, path, address(this), block.timestamp);
    }
    ...
    function _swapEthOut(
        ...
        _safeApproveHelper(startToken, uniswap, balance);
        IUniswapRouterV2(uniswap).swapExactTokensForETH(balance, 0, path, address(this), block.timestamp);
    }
